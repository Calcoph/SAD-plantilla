...
import plantilla_SAD
from plantilla_SAD import ImputeMethod, ImputeFeature, get_att_default, ColumnType, RescaleFeature, RescaleMethod, coerce_to_unicode, Algorithm
...
if __name__ == '__main__':
    ...
    (
        ...
        OVERSAMPLING_RATIO,
        BINNING_COLUMNS
    ) = plantilla_SAD.get_config(config)
    ...
    testX = plantilla_SAD.preprocess(
        ml_dataset,
        COLUMNS,
        PUNTO_A_COMA_COLUMNS,
        CATEGORICAL_COLUMNS,
        ...
        PREDICT_COLUMN,
        TARGET_MAP,
        plantilla_SAD.json_bool(get_att_default(algorithm, "preprocess_numerical", "true")),
        binning,
        BINNING_COLUMNS,
        binning_arg
    )
    ...
    if algorithm["algorithm"] == Algorithm.KNN:
        drop = plantilla_SAD.json_bool(get_att_default(algorithm, "drop", "true"))
        impute = plantilla_SAD.json_bool(get_att_default(algorithm, "drop", "true"))
        rescale = plantilla_SAD.json_bool(get_att_default(algorithm, "rescale", "true"))
    elif algorithm["algorithm"] == Algorithm.DecisionTree:
        drop = plantilla_SAD.json_bool(get_att_default(algorithm, "drop", "true"))
        impute = plantilla_SAD.json_bool(get_att_default(algorithm, "drop", "true"))
        rescale = plantilla_SAD.json_bool(get_att_default(algorithm, "rescale", "false"))
    elif algorithm["algorithm"] == Algorithm.RandomForest:
        drop = plantilla_SAD.json_bool(get_att_default(algorithm, "drop", "true"))
        impute = plantilla_SAD.json_bool(get_att_default(algorithm, "drop", "true"))
        rescale = plantilla_SAD.json_bool(get_att_default(algorithm, "rescale", "false"))
